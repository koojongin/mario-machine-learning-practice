...
class DQNAgent:
    ...
    def build_model(self):
        ...
        self.input = tf.placeholder(
            dtype=tf.float32, shape=(None,) + self.states, name="input"
        )
        ...
        with tf.variable_scope("online"):
            ...
            self.dense = tf.layers.dense(
                inputs=self.flatten, units=512, activation=tf.nn.relu
            )
            self.output = tf.layers.dense(
                inputs=self.dense, units=self.actions, name="output"
            )
        # Target network
        with tf.variable_scope("target"):
            self.conv_1_target = tf.layers.conv2d(
                inputs=self.input_float,
                filters=32,
                kernel_size=8,
                strides=4,
            ...
            self.conv_2_target = tf.layers.conv2d(
                inputs=self.conv_1_target,
                filters=64,
                kernel_size=4,
                strides=2,
                activation=tf.nn.relu,
            )
            self.conv_3_target = tf.layers.conv2d(
                inputs=self.conv_2_target,
                filters=64,
                kernel_size=3,
                strides=1,
                activation=tf.nn.relu,
            ...
            self.dense_target = tf.layers.dense(
                inputs=self.flatten_target, units=512, activation=tf.nn.relu
            )
            self.output_target = tf.stop_gradient(
                tf.layers.dense(
                    inputs=self.dense_target, units=self.actions, name="output_target"
                )
            )
        # Optimizer
        self.action = tf.argmax(input=self.output, axis=1)
        self.q_pred = tf.gather_nd(
            params=self.output,
    ...
    def save_model(self):
        ...
        self.saver.save(
            sess=self.session, save_path="./models/model", global_step=self.step
        )
    ...
    def predict(self, model, state):
        ...
        if model == "online":
            return self.session.run(
                fetches=self.output, feed_dict={self.input: np.array(state)}
            )
        if model == "target":
            return self.session.run(
                fetches=self.output_target, feed_dict={self.input: np.array(state)}
            )
    ...
    def learn(self):
        ...
        if self.double_q:
            ...
            target_q = (
                reward
                + (1.0 - done) * self.gamma * next_q[np.arange(0, self.batch_size), a]
            )
        else:
            target_q = reward + (1.0 - done) * self.gamma * np.amax(next_q, axis=1)
        # Update model
            ...
            for _ in range(n_replay):
                ...
                while True:
                    ...
                    if plot:
                        if step % 100 == 0:
                            self.visualize_layer(
                                session=sess, layer=self.conv_2, state=state, step=step
                            )
                    # Action
                    if np.random.rand() < 0.0:
                        ...
                        q = sess.run(
                            fetches=output, feed_dict={input: np.expand_dims(state, 0)}
                        )
                        action = np.argmax(q)
